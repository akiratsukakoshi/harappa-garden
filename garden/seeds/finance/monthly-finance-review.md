...
execute:
  ...
  prompt: |
    ...
    Step 1.5 クライアント突合(toB の soil × Freee 部門売上・S52 で追加):
      {PY} {ANA} client-recon
      → soil クライアント台帳の案件ステータスと、Freee の toB 部門(企業案件/共創プロジェクト)
        売上を月別に並べ、「投げかけ候補」(入金待ち / 請求済だが freee反映 未確定)を出す。
      重要な前提(突合で判明):
        - Freee の売上は取引先タグなし → 部門で toB を識別(企業案件=研修等 / 共創=みうらの森林型)
        - 京急みうらの森林は freee_partner_role=支払先(外注費=コスト)→ 売上に数えない
        - 三井 CACR1 は前受金(入金済・月次取り崩し)/ ゴンチャ茶畑は利益なし(売上ゼロ)
      「投げかけ候補」に出た案件(入金待ち・要確認)を Step 2 の論点に取り込む。

    Step 1.6 案件ステータスの鮮度チェック(④ S57 = 月次サイクルに組込):
      Step 0 で読んだ toB-pipeline.md の各案件(確度 / freee反映 / payment_status)を、
      Step 1.5 client-recon の Freee 実態と突き合わせ、**ズレ・進展した案件を列挙**する。例:
        - 入金待ち → 着金済(freee反映 ⏳→✅)
        - 見込み → 確定(受注が確定した案件)
        - 請求済だが freee 未反映(記帳漏れ疑い)
      この種は read-only なので **soil は書き換えない**。動いた案件は Step 2 の投げかけに
      「今月ステータスが動いた案件 → soil 更新の提案」として乗せ、確定後に対話で
      toB-pipeline.md + 該当 [clients/{企業}/projects/{案件}/README.md] frontmatter を更新する
      (= client_steward の承認境界と同じ:解釈の soil 反映は剪定経由)。
      ※ 動きが無ければこの Step は「ステータス変化なし」で省略してよい。
    ...
    Step 2 投げかけを作る(★この種の本質):
      ...
      ガクチョと一緒に考えたい論点を 1〜2 点添える。例:
        ...
        - CF 安全性(資金ショート月の有無)
        - ★toB クライアントの入金待ち/要確認(client-recon の投げかけ候補)
