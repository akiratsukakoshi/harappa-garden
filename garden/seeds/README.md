...
last_updated_by: claude (with ガクチョ, セッション60 — 測量士 2026-06-24 提案5)
---

# garden/seeds/ — 種(自律トリガー)

> **種** = 「いつ・何が起きて・何を動かし・誰に剪定依頼するか」を1ファイル1ジョブで宣言した起動装置。
> 各種は HMC SKILL 群を **Garden Runtime(seed runner)のヘッドレス起動** で呼び出す(現状の runner は `claude -p`。launcher が `engine` を解決し、将来 codex / gemini-cli に切替可能)。庭師は剪定(承認/修正/却下)だけに専念する。
...
## 実行ホスト(セッション7 確定 / S53 で local 例外を追加)

- **原則: cron 系の種は VPS 上で起動する**。塚越さん PC の起動状態に依存しない構造を取る。
- セッション6 ADR「種の頭脳 = seed runner のヘッドレス起動 on VPS」の延長線上の決定。
- WSL 上の cron は原則 **使わない**(PC 夜間ダウン → 早朝・夜間の cron が走らない問題を構造的に排除)。
- **例外: `execution_host: local` の種はローカル WSL の crontab で起動する**。種 frontmatter の `execution_host`(`vps`(既定) | `local`)で宣言する。
  - 現状の唯一の例外は [scribe/daily-recording-sweep](scribe/daily-recording-sweep.md)。Plaud MCP トークン(`~/.plaud/tokens-mcp.json`)を持つホストをローカル WSL 1 つに固定しているため(refresh_token ローテートの都合で所有ホストを増やさない)。実行は [run-local.sh](../services/scribe/run-local.sh) 経由で、ローカルで soil/board を書いてから VPS へ push する。詳細は [ADR 2026-06-17](../../docs/decisions/2026-06-17-plaud-mcp-project-side.md)。
...
execute:
  ...
  prompt: |                       # seed runner(LLM)に渡す自然言語指示
    対象月 = …
    手順:
      1. …
      2. …
...
- seed runner(現状 `claude -p`)を VPS でシェル経由で起動する想定(常駐 daemon ではない)。

### event 種

- watcher daemon が glob を監視。マッチ → seed runner(現状 `claude -p`)起動。
