...
| `CLAUDE_BIN` | `~/.npm-global/bin/claude` | `claude-code` runner のバイナリ |
| `CLAUDE_TIMEOUT_MS` | `600000`(10 分) | seed runner のタイムアウト(全種共通の既定) |
| `GARDEN_CRON_SECRETS` | `~/.claude/cron-secrets.env` | cron 実行時に runner 用 secret を注入する env ファイル |

### engine / runner

種 frontmatter の `engine` を launcher が解決する。現状の実装済み runner は `claude-code` のみ。
`codex` / `gemini-cli` など未対応 engine を指定した場合は、黙って `claude-code` に戻さず exit 2 で明示エラーにする。
...
未指定なら `CLAUDE_TIMEOUT_MS`。タイムアウト時は runner process が SIGTERM され exit 143 になる。
...
→ frontmatter パース + 変数評価 + prompt 構築までを実行。runner は呼ばずログに `[dry-run]` を記録。
