...
def build_dialogue_prompt(convo: str, user_text: str, now: datetime.datetime) -> str:
    ...
    if any(t in f"{convo}\n{user_text}" for t in SNS_TOPICS):
        ...
        sns_block = (
            ...
            + "・board dashboard は閲覧専用。ガクチョが Discord で一言コメントや差し替えを伝えたら、該当 sns-select board を Edit して反映する。\n"
            ...
            + "・SNS の後追い承認に注意。現在が月曜 07:30 を過ぎていて sns-select board を今 approved にした場合、"
            + "「月曜朝に走ります」とは返さない。自動 cron の窓は過ぎているため、"
            + "/home/vps-harappa/garden/services/launcher で `node launcher.mjs --seed sns_manager/monday-caption-draft` を手動起動し、"
            + "board/pending/*-sns-caption.md ができたこと、または起動できない理由を確認してから返す。\n"
    ...
    return (
        ...
        + f"[ガクチョの新しい発言]\n{user_text}\n\n"
        + "[空返事禁止・実行確認ルール]\n"
        + "・「反映した」「作成した」「予約した」「送った」「進めます」と言う前に、対象ファイル・ログ・標準出力・外部 API の結果など、"
        + "少なくとも 1 つの客観的な完了根拠を確認する。\n"
        + "・完了根拠を確認できない場合は、完了したふりをしない。「まだ動いていない」「ここで止まっている」「この権限/入力が足りない」と返す。\n"
        + "・定時 cron 前提の workflow を、時刻を過ぎてから後追いで動かす場合は、次回 cron 待ちにせず、手動起動が必要かを判断する。"
        + "手動起動できない場合はその理由と次の操作を返す。\n"
        + "・エラーやスタックポイントを見つけたら、最小限でよいので「どの board/log/command で止まったか」を添える。\n"
        + "・内部エラー、tool エラー、権限不足、対象 board 不在、依存サービス停止、cron 窓経過は、黙らずにそのまま伝える。\n\n"
