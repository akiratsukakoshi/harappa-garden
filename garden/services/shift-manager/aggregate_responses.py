#!/usr/bin/env python3
"""aggregate_responses.py - 翌月シフトアンケート(Google Forms)の回答を集計し、
シフト調整シート(shift_work)に NG マトリクスを書き出す。

HMC apps/shift_manager/logic/aggregate_responses.py から Garden 化。
変更点(業務知識は継承、起動と承認だけ Garden に変える):
- パスを __file__ ベースの絶対パスに(cwd 非依存)
- credentials/token は SCRIPT_DIR/secrets/ 配下、config は SCRIPT_DIR/config/ 配下
- import: modules → lib
- (バグ修正)死んだ重複 aggregate() スタブを削除
- (バグ修正)__main__ の `datetime.timezone.utc` → `timezone.utc`(datetime はクラスで .timezone を持たない)
- それ以外のロジックは HMC 版と完全同一

使い方:
    .venv/bin/python aggregate_responses.py --month 2026-07
    (--cutoff YYYY-MM-DD で回答の取り込み開始日を上書き / 既定は前月1日 UTC)
    (--output_suffix で出力シート名末尾を変える = dry-run 的に別タブへ書ける)
"""
...
if __name__ == "__main__":
    ...
    cutoff = None
    if args.cutoff:
        cutoff = date_parser.parse(args.cutoff).replace(tzinfo=timezone.utc)
