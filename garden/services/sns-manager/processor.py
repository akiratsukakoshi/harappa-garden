...
def _clear_stale_candidate_dirs():
    """temp/candidates-* を全削除。古い候補画像が残ると「fetch 失敗時に古い画像で投稿を
    再作成する」事故の元になる(S56 で実害確認)。毎回フレッシュ取得だけを残す構造ガード。"""
    import shutil
    ...
    for name in os.listdir(TEMP_DIR):
        if name.startswith("candidates-"):
            ...
            if os.path.isdir(path):
                ...
                logger.info(f"古い候補ディレクトリを削除(stale fallback 防止): {name}")
...
def cmd_fetch_images(week: str):
    ...
    # 先に古い候補を一掃 → この後に何も DL されなければ temp は空(古い画像の再利用が物理的に不可能)
    _clear_stale_candidate_dirs()
...
def _resolve_drive_urls(paths):
    """各ローカル画像を Drive 原本(候補フォルダ直下 → 使用済み/)で引き当て、
    公開化した失効しない Drive 直リンクのリストを返す。

    1枚でも Drive 原本が引けない(アドホックでローカル画像を直接渡した等)場合は
    None を返し、呼び出し側は FB CDN フォールバックに切り替える。
    """
    folder_id = os.getenv("SNS_DRIVE_FOLDER_ID")
    if not folder_id:
        return None
    from lib.drive_reader import DriveReader
    reader = DriveReader()
    used_id = reader.find_used_folder_id(folder_id)
    urls = []
    for p in paths:
        name = os.path.basename(p)
        found = reader.find_image_by_name(folder_id, name)
        if not found and used_id:
            found = reader.find_image_by_name(used_id, name)
        if not found:
            logger.info(f"Drive 原本が見つからない → FB CDN フォールバック: {name}")
            return None
        reader.make_public(found["id"])
        urls.append(DriveReader.public_download_url(found["id"]))
    return urls
...
def cmd_schedule(image, images, caption_file, publish_at, platform, dry_run, no_archive=False):
    ...
    if platform in ("both", "ig"):
        ig_urls = _resolve_drive_urls(paths)
        if ig_urls:
            # 通常フロー: 失効しない Drive 公開URL で予約(数日先でも安全)
            if len(ig_urls) > 1:
                result["ig_id"] = meta.schedule_ig_carousel_via_server_url(ig_urls, caption, dt)
            else:
                result["ig_id"] = meta.schedule_ig_photo_via_server_url(ig_urls[0], caption, dt)
        else:
            # アドホック: Drive 原本なし → FB CDN フォールバック(URL は数日で失効)
            logger.warning("Drive 原本を引けない → FB CDN URL で予約(数日先の予約は失効リスク)")
            if len(paths) > 1:
