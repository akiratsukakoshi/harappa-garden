...
4. Discord に承認依頼通知 → ガクチョが Discord で**返信・差し替え・一言コメント・承認**を伝える(日曜夜まで)

> ガクチョは dashboard で board を見ながら、Discord で各画像の**一言コメント**をガクコに伝える(= 月曜の文案生成の起点)。dashboard は閲覧専用で、直接入力しない。
