...
- 配信予定の LINE 本文(編集可、コードブロックで囲む)
  - 🔴 **回答期限は配信月の7日**(S40 確定。7日締切 → 8日に Mode 4 集計 → 10日シフト確定の月内サイクル)
- フォーム URL
- 配信先(staff グループ)
- 配信タイミング(承認後すぐ / 時刻指定)
- 庭師アクションガイド(承認/修正/却下)
...
## Mode 4: Month-day-8 Aggregate(月初8日 — シフト回答集計 → 10日確定)

**起動**(2 ルート):
- **cron**: 種 `shift_manager/monthly-shift-finalize`(draft、cron 毎月8日 08:00。S40 起草、初回見届けで active)。アンケート締切(7日)の翌朝に集計し、**10日のシフト確定**に間に合わせる
- **手動**: ガクチョが Discord master で「**シフト集計まわして**」(対象月指定可: 「8月分の〜」)→ ガクコが scoped Bash で Step 1 のコマンドを直接実行して結果報告。**遅れ回答の再集計**に使う(マージ安全、何度でも可)

月内サイクル: 1日 アンケート開始(Mode 2)→ 7日 締切 → **8日 集計(本 Mode)** → 10日 シフト確定(庭師、シフト調整シート上)
...
- 種 `monthly-shift-finalize` は **draft 起草済(S40)**。cron 未登録のため当面は手動実行(S37 で 7 月分を実集計 = 11 名)。Step 2 は構想のまま。
...
- [x] 種 `monthly-shift-finalize` draft 起草(S40 完了。[garden/seeds/shift_manager/monthly-shift-finalize.md](../../seeds/shift_manager/monthly-shift-finalize.md))
- cron 登録 + 初回見届け → active 昇格
...
2. **キーワードで推定**: ガクチョの指示に「シフト募集」「アンケート」(=monthly-shift-survey)、「稼働確認」「稼働表配信」(=monthly-working-hours-confirmation)、「集計」「prep」「稼働まわして」(=month-end-working-hours-prep)、「シフト集計」「シフト確定」(=monthly-shift-finalize)等のキーワードが含まれていれば、from_seed で絞り込み。
   ※「シフト集計まわして」は board 承認ではなく **Mode 4 の手動実行ルート**(aggregate を直接実行)の合図でもある。pending に monthly-shift-finalize board があればその承認として扱い、無ければ手動実行として扱う。
...
- 「翌月シフトアンケートの配信文、下書き作りました。回答期限は 6/7(日)です。文面はそのままでよさそうですが、配信時刻だけ指定してください。今すぐ / 朝9時 / その他 のどれにしますか?」
...
| `garden/seeds/shift_manager/monthly-shift-finalize.md` | 月初8日 08:00 cron + 手動「シフト集計まわして」(draft、S40 起草) | Mode 4 Step 1 |
