---
name: finance
description: 原っぱ大学の財務区画。売上記帳(STORES/Square CSV → Freee 振替伝票)・データ整合性の地ならし(部門漏れ + 未登録明細)・財務分析(PL/CF/着地予測の対話)を月次サイクルで束ねる。さらにクライアント向けの見積書/請求書を案件台帳から雛形 .xlsx で発行する(Mode G・S55 新植)。HMC の finance_importer / freee_auditor / finance_analyzer の transplant 移植。master / Discord 完結。
plot: finance
topics: [財務, 売上記帳, 振替伝票, STORES, Square, 売上CSV, freee, 勘定科目, 部門, 部門漏れ, 監査, データ整合性, 未登録明細, 口座明細, wallet_txns, PL, 損益, キャッシュフロー, CF, 着地予測, 財務分析, 戦略議論, 試算表, 営業利益, 販管費, 目標, 見積書, 請求書, 発行, 雛形, issue_document, 明細パターン, finance_importer, finance_analyzer, freee_auditor]
inherits_from:
  - garden/CHARTER.md
...
created: 2026-06-17
last_updated: 2026-06-19   # S55: Mode G(見積/請求の発行)を新植
...
linked_services:
  - garden/services/finance (importer.py / auditor.py / analyzer.py / issue_document.py)
...
| **G 発行(issue)** | 新規(S55 seedling) | 見積/請求の構造正本 md → 雛形 .xlsx に差し込み発行 | 連携なし(発行物のみ) |
...
## Mode G: 発行(見積 d / 請求 f の構造正本 → 雛形 .xlsx)

**起動**: 対話のみ(「{案件}の請求書つくって」「{案件}の見積つくって」)。**種(cron)は無し** — 発行は不定期・案件起点なので自律トリガーを置かない。

> 役割 = クライアントに渡す見積/請求を、案件台帳の実額から正確・一貫した様式で起こす。**プロセス md が soil に残る**(cowork に外注せず Garden 内で完結 = ナレッジ蓄積 + ベンダーロックイン回避。知識=md・型=md・差し込み=openpyxl のみで、LLM/SaaS に依存しない)。

### フロー
1. **案件台帳を読む** — `soil/clients/{社}/projects/{案件}/` の README(finance_links / 合意額 / 明細)。**金額の正本は案件台帳**。
2. **型を選ぶ** — [明細パターン.md](../../soil/finance/templates/明細パターン.md) で案件種別(研修・単発 / 継続運営)の明細骨格を下敷きに。
3. **構造正本 md を起草** — soil テンプレ frontmatter 形式([見積](../../soil/finance/templates/見積書テンプレート.md) / [請求](../../soil/finance/templates/請求書テンプレート.md))で `doc / client / subject / 日付 / line_items` を埋める。発行元・振込先・登録番号は雛形に固定で入っているので md には書かない([harappa-billing.md](../../soil/finance/harappa-billing.md))。
4. **★ガクチョに提示して確認(承認境界)** = 宛先・件名・明細・単価・数量・日付・合計を 1 画面で見せ、**OK が出るまで .xlsx を出さない**。明細をまとめるか割るか・単価・有効期限/振込期限をここで詰める。
5. **発行** — `issue_document.py {md} --out {社}様_{件名}_{見積|請求}.xlsx`。出力先は案件台帳の `d見積/` or `f請求/`。md も同じ場所に残す(構造正本 = 次回の下敷き + 監査証跡)。
6. **ガクチョが Excel で開いて社印を捺印 → PDF → 送付**(PDF 化は手作業)。
7. **請求の場合**:着金後 → **Mode I で freee 記帳にバトン**、案件台帳の `freee反映` を ✓、[toB-pipeline](../../soil/projects/toB-pipeline.md) の着地予測に反映。

### コマンド
```
python3 issue_document.py {md}                    # md と同じ場所に .xlsx
python3 issue_document.py {md} --out {path.xlsx}  # 出力先を指定
python3 issue_document.py {md} --dry-run          # 保存せず税抜小計を検算
```
- 雛形 = `services/finance/templates/{invoice,estimate}-template.xlsx`(社印は 0 画像 = ガクチョが捺印)。**雛形の数式(小計・消費税・合計の SUM)は触らず再計算に任せる**。
- 明細上限 = 請求 22 行 / 見積 16 行(超えたらまとめる)。

### 判断ルール(発行固有)
- **でっち上げない**:案件台帳に金額・明細が無ければ、何が足りないかを示して止まる(推測で数字を埋めない)。
- **内部原価(粗利・利益率)は発行物に載せない**([明細パターン.md](../../soil/finance/templates/明細パターン.md) の内部試算ブロック参照)。
- **酒類など外税扱い**は雛形が一律 ×0.1 で計算するので、発行後にガクチョが当該行の消費税を手調整。
...
| 発行(Mode G) | 案件台帳に金額が無ければ止まる(でっち上げない)。内部原価は発行物に載せない。確定 md をガクチョが OK するまで .xlsx を出さない |
...
- **発行**: `📄 構造正本(明細・合計)` → `🔖 これで発行していい?` → `📎 .xlsx 出力(開いて捺印 → PDF)`
...
### Mode G(発行)の状態 — S55 新植
- 段階: **test**。issue_document.py + 社印入り雛形2つ + 型ライブラリ([明細パターン.md](../../soil/finance/templates/明細パターン.md))+ 実出力 GREEN(MTI 請求15行 / 三菱UFJ 見積6行、金額が実物一致)+ **ガクチョ目視 OK**(和暦表示・社印保持・件名ボールド)。
- active 条件: 実案件で 1 本発行 → ガクチョが捺印・PDF・送付 → active。
- **ローカル専用**(発行は Claude Code セッション=Opus で対話起動。VPS デプロイ不要。`Pillow` が社印保持に必須 = requirements に追加済)。
