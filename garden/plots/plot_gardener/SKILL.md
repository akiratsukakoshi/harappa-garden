...
### board / notification(board を作る種は board/README.md 正本に従う)
- board を作る条件:
- board_registry.py に登録したか(kind/title/approval/dispatcher):
- board_lint.py 違反なしを確認したか:
- Discord / LINE 通知:
- 失敗時:
...
- **board を作る種は [board 運用の正本](../../board/README.md) に必ず従う**(S56 一元管理):
  - frontmatter は正本のスキーマ(`type`/`from_seed`/`title`/`status`)に合わせる。
  - **[`board_registry.py`](../../services/garden-gaku-co/board_registry.py) に1行登録**
    (`kind`/`title`/`approval`[AUTO/CONVERSATIONAL/FYI]/`dispatcher`)。登録しないと
    承認時に「未登録の board 種」で弾かれ、`board_lint` が ERROR を出す。
  - 不可侵原則(古いデータにフォールバックしない / pending=承認待ちだけ / 日本語タイトル)を守る。
