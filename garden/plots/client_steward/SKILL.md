...
### 手順
1. 各 active client の `primary_domain` で Gmail を `last_synced` 以降だけ検索 → 新着スレッドの要点を **`clients/{slug}/_inbox.md`(生取り込みレーン)に append**(`sweep_client --write-inbox`、append-only・thread_id dedup・自動)。
2. Plaud を名前で `last_synced` 以降検索 → 新録音の note を `meetings/` に append(対話時 MCP。cron 非対象=homework)。
3. **filing(人)**: `_inbox.md` の各行を、属する `projects/{案件}/emails/` に振り分ける。**スレッド→案件の束ね(クラスタリング)は機械に渡さない**([ADR 2026-06-18](../../../docs/decisions/2026-06-18-client-bootstrap-manual-not-scaffold.md)= 京急で破綻実証)。filing 済みの行は `_inbox.md` から消してよい(state で dedup 済=再 append されない)。
4. **living 台帳(README)を更新** + watermark 更新。
5. **変化と要フォローを digest 通知**(Discord master):新日程確定 / 見積→発注 / 請求→入金 / 未返信◯日 / 入金期限接近 / 新規案件の芽。
6. **解釈は board 提案**(下記 承認境界)。

> **生取り込みレーンの設計(S57)**: emails/ は案件単位なので「機械が emails/ に直接 append」はクラスタリング判断を機械に強いる。そこで **client 直下の整理前バッファ `_inbox.md`** に分離 = 機械は append-only(判断ゼロ・上書き事故ゼロ)、案件への filing は人。「真の一発 bootstrap」の安全な第一歩。
...
| 動作 | 扱い |
|---|---|
| 生取り込み(新メールの**要点を `_inbox.md` に append**) | **自動でよい**(append-only・thread_id dedup。`--write-inbox`) |
| filing(`_inbox.md` の行を `projects/{案件}/emails/` に振り分け) | **人**(スレッド→案件のクラスタリング判断) |
| 解釈(**確度の変更・案件の統合/新規確定・freee反映の断定**) | **board → ガクチョ剪定 → soil 反映** |
| 担当者の実名確定 | メール署名は自動採用可。Plaud 話者は採用しない(不確か) |

> append-only レーン(`_inbox.md` 生取り込み → 人が filing → `emails//meetings/`)+ living 台帳(README が「今の真実」)。機械は判断ゼロの append だけ、束ねと解釈は人/board。
