...
- **直近の宿題(最優先)**: **(ガクチョ宿題・出先で保留中)** 社内 LINE サーバ**デプロイ** = VPS rsync + venv-line(私)→ ⭐secret 記入(ガクチョ、API key 手配済)→ ⭐NPM 社内サブドメイン → 起動 + `/health` → ⭐Webhook Verify → ⭐グループ追加 → groupId 確定 → 実発話疎通。手順 = [DEPLOY.md](services/garden-gaku-co/DEPLOY.md) / **(次フェーズ)** core_team 業務 tool 整備 = 測量士 [plot_gardener 手紙](../docs/surveyor/letters/2026-06-03.md)参照 / **(明朝・材料待ち)** ingest-raw `==NOTIFY==` 初検証 / **(継続)** calendar token 寿命確認(約 6/5)/ 測量士の次回手紙 / kura 区画整備
