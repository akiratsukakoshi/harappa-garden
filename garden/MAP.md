...
- **実装フェーズ**: S31 で**ガクコ社内チャンネル参加(運営スタッフ LINE = core_team)の方針確定 + ベンダー中立対話層 ADR + 中立基盤実装 + LINE チャネル開設**。発見:gaku-co5.0 は既に LINE 対話+メンション制御+scope 記憶を実装済み・稼働中で、S22 統合 ADR の前提が逆転していた。ガクチョ指摘で**社外(aibou/デジタル原っぱ)と社内のエアギャップ必須**を再確認(=2026-05-28 ADR 決定4 が既に明文化済み)。**ベンダー中立 = API を使わないでなくアダプタ 1 枚で隔離**を ADR 化 → 中立基盤(brain/provider.py + tools/registry.py + capabilities.py)を薄い実スライスで実装・smoke GREEN。LINE 社内チャネルをサーバ無しでできる所まで開設(secret/token 取得・応答オフ・グループ許可、Webhook URL はサーバ構築時)
- **直近セッション**: [2026-06-03 セッション31](../docs/sessions/2026-06-03-session31.md) — **ガクコ社内 LINE 参加の方針確定 + ベンダー中立 ADR + 中立基盤 + LINE 開設**:(0) S30 宿題 cron 検証(ingest-raw コア成功だが NOTIFY 欠落発見 / night-cheer・consolidate cron は未来)(1) ingest-raw の NOTIFY/log 方式を consolidate-wiki と統一(stdout+`==NOTIFY==`・log 直書きやめ、SKILL+種 3 箇所修正・VPS 反映)(2) 見積もりをエージェント時間で出す feedback を memory 保存 (3) **B 検討で gaku-co5.0 が社外も担うと判明 → 私の soil/memory 共有案を撤回、社外/社内エアギャップへ。過去ログ 2026-05-28 決定4 で確認。頭脳=チーム b(API Haiku、理由=ゲート経済/同時セッション回避/reply速度/capability 情報境界)。ベンダーロックイン懸念で「skill=SDK tool」の誤解を 3 層分離で訂正** (4) ADR 起草 + 2026-05-31 ADR・MAP の「撤退→社外専用化」訂正 + 中立基盤実装(smoke GREEN)(5) LINE 社内チャネル開設(伴走)
...
- **直近の重要決定**: S31 = ADR 1 本 [2026-06-03 vendor-neutral-interaction-layer](../docs/decisions/2026-06-03-vendor-neutral-interaction-layer.md)(対話層の 3 層分離 + LLM アダプタ + 中立ツール/capability + 社外/社内エアギャップ再確認)。正本は [2026-05-28 決定4](../docs/decisions/2026-05-28-garden-gaku-co-interaction-layer.md)、[2026-05-31 統合 ADR](../docs/decisions/2026-05-31-garden-gaku-co-unification.md) は「撤退→社外専用化」に訂正注記
- **直近の宿題(最優先)**: **(次回・API key 手配済)** 社内サーバ構築 = 中立基盤の上に LINE webhook(FastAPI、gaku-co5.0 移植)+ Stage1 メンションゲート + core_team persona/memory + tool → VPS デプロイ + NPM 社内専用サブドメイン → Webhook URL を LINE に入力 → Verify → グループ追加。secret/token は VPS `.env`(600)にガクチョ直貼り / **(今晩 22:40)** night-cheer(S28 修正後初検証)/ **(明朝 03:30)** ingest-raw が log に `==NOTIFY==` を残せるか(S31 修正の初検証)/ **(明朝 03:50)** consolidate-wiki cron 初稼働(S30 新設)/ **(継続)** calendar token 寿命確認(約 6/5)/ 測量士の次回手紙 / kura 区画整備
