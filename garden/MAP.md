...
- **直近セッション**: [2026-06-10 セッション40](../docs/sessions/2026-06-10-session40.md) — **宿題一掃 + シフト集計 8日化**:S39 レビュー残 6 本完遂(genai 移行 / finalize 種 / 命名 ADR 参照 / _archive 整理 / launcher unit test / 番人朝統合)+ 廃止 run-line-server.sh・venv-line 削除 + シフトサイクル「1日→7日締→8日集計→10日確定」確定 + 手動「シフト集計まわして」(bot Route A)+ バグ2件(shift Bash `:*` / `$()月` 未展開 4 種)+「翌々月」→「翌月」統一
