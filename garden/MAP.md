...
- **直近の重要決定**: S31 ADR [2026-06-03 vendor-neutral-interaction-layer](../docs/decisions/2026-06-03-vendor-neutral-interaction-layer.md)(対話層 3 層分離 + LLM アダプタ + 中立ツール/capability + 社外/社内エアギャップ)の **実装順序「2」を S32 で消化**(コード完成・ローカル検証 GREEN)。正本は [2026-05-28 決定4](../docs/decisions/2026-05-28-garden-gaku-co-interaction-layer.md)
- **直近の宿題(最優先)**: **(次フェーズ・本命)** plot_gardener の **dogfood 第 1 号を選定 → 区画化を 1 件完走 → test→active 昇格**(候補: shift_manager=型検証 / expense_processor=実利・移植本番 / core_team の LINE read tool 群=デプロイ直後の価値)/ **(ガクチョ宿題・出先で保留中)** 社内 LINE サーバ**デプロイ** = VPS rsync + venv-line(私)→ ⭐secret 記入(ガクチョ、API key 手配済)→ ⭐NPM 社内サブドメイン → 起動 + `/health` → ⭐Webhook Verify → ⭐グループ追加 → groupId 確定 → 実発話疎通。手順 = [DEPLOY.md](services/garden-gaku-co/DEPLOY.md) / **(明朝・材料待ち)** ingest-raw `==NOTIFY==` 初検証 / **(継続)** calendar token 寿命確認(約 6/5)/ 測量士の次回手紙 / kura 区画整備
...
| 区画 (plots) | [garden/plots/](plots/) | 🌱 | **第1号 daily-pilot(S19)+ 第2号 shift_manager(S21)+ メタ区画 plot_gardener(S33)**:いずれも CHARTER 継承型 |
| 区画-plot_gardener ← **NEW** | [garden/plots/plot_gardener/](plots/plot_gardener/) | 🌱 | **S33 新設(メタ区画)**:業務を区画化する型。Mode 1〜6(Intake/Legacy Inventory/Workflow Spec/Garden Design/Implementation Plan/Review&Promotion)+ transplant/seedling/hybrid。測量士サンプル起点。状態 **test**(最初の 1 業務に当てて active へ)。[ADR](../docs/decisions/2026-06-04-plot-gardener-and-vocabulary-registers.md) |
