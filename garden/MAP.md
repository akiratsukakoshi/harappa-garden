...
| 区画-expense_processor | [garden/plots/expense_processor/](plots/expense_processor/) | 🌱 | **S35 新設 plots 第3号(plot_gardener 初 dogfood / transplant)/ S37 Phase 2 完了**:Mode 1 月末リマインド / Mode 2 抽出→board / Mode 3 承認→dry-run→Freee 登録。**scope=master 構造遮断**。状態 **draft**(Phase 2 = service 移植 + secret + tax_code 136 + dry-run + cron まで S37 完了。**残=初回リアル実走で active**) |
| サービス-expense-processor ← **NEW** | [garden/services/expense-processor/](services/expense-processor/) | 🌳 | **S37 移植 + VPS デプロイ**:processor.py(extract/upload/taxes)+ lib/freee_client フル + drive_client + utils。Drive=service account / Freee token=shift-manager 共有(`FREEE_TOKEN_FILE`)/ Gemini=`gemini-2.5-flash`(2.0 退役対応)/ tax_code 136。dry-run GREEN(合成データ)。data/secret は VPS のみ |
