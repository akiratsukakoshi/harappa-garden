...
- **実装フェーズ**: S24 で **board ライフサイクル完成(自動承認依頼通知 + 連鎖解除)+ 朝のブリーフィングへの board リマインド統合 + 放サボ自動取込のフルパス開通 + コドモン CSV 運搬路 α 確立** を一気通貫。S21 から潜伏していた「種 → board → 配信」の 3 つの構造穴(Bash 未許可・通知モック放置・連鎖未自動化)+ 放サボ取込の 3 重バグ(時間空スキップ・運搬路欠落・ファイル名固定)をすべて塞いだ。 **5 月稼働確認 + 7 月シフトアンケートが同夜中に完走、放サボ列 12 セル反映成功**
- **直近セッション**: [2026-06-01 セッション24](../docs/sessions/2026-06-01-session24.md) — **board ライフサイクル完成 + 承認依頼通知 + 放サボ自動取込のフルパス開通**:dummy 配信不在発覚 → settings.json Bash 許可追加 + send_pending.py に notify_pending / unblock_confirmation 関数追加 + board/failed/ 新設 + 壊れた board 退避 / morning-briefing に Step 1.5「承認待ち board リマインド」追加 / 種 3 本に庭師アクション統一テンプレ強制 + extract_send_body 部分一致緩和 + confirmation の `## 配信本文` セクション必須化 / 5 月の放サボ深掘りで generate_working_hours.py の致命バグ発見・修正 + import_kodomon.py のファイル名柔軟化 + WSL → VPS CSV 運搬路 α 新設 / 既存タブ再生成 → 放サボ列 6 列生成 + CSV 取込 12 セル反映成功
- **直近の重要決定**: **board ライフサイクル拡張 ADR**(決定 7 つ: failed/ 新設・blocked/notified_at 正式化・通知 send_pending 統合・朝の Triage 統合・装飾見出し許容・庭師アクション統一・CSV 運搬路 α 採用) / **チェックボックス = 備忘録、発火 = `status:` 一本化**(S21 の設計乖離を解消) / **放サボイベント = 時間空でも events に追加**(import_kodomon が後追いで CSV から値書き込む前提を実装に反映) / **コドモン CSV 運搬路 = α(WSL cron + rsync)暫定、γ(Discord アップロード)を将来構想として MAP/ADR にメモ**
- **直近の宿題(最優先)**: **(明朝 06:30 初稼働)** morning-briefing の board pending リマインド(Step 1.5)が active_tasks 末尾に出るか確認 / **(明朝以降)** ガクチョが Discord master に届いた 2 通(7 月シフトアンケート + 5 月稼働確認)を staff LINE グループに手動コピペ配信(dummy モード継続) / **(次セッション)** 永続記憶 Stage A.5 実装(S23 で 4 論点合意済) + soil 正本関係の明文化(S23 持ち越し) + γ 移行検討 / **(継続宿題)** NPM 認証懸念(統合 Stage 6 で同時解消予定)+ calendar token の 7 日後寿命確認(約 6/5)
...
| 配信ディスパッチャ | [services/garden-gaku-co/send_pending.py](services/garden-gaku-co/send_pending.py) | 🌳 | **S21 新設 + S24 大幅拡張**:cron 1分毎、status: approved 検知 → dispatch、 **status: pending → 承認依頼通知(notified_at 冪等化)、shell 種成功時 → 連鎖 confirmation の unblock**、scheduled_send 待機、status:test テスト配信 |
