...
- **実装フェーズ**: S30 で永続記憶の 3 ステージ(A.5 + B + C)が全て active + 動的再読み込み機構で対称化。**1 = Stage B Consolidate**(launcher 1.0.1 で model パラメータ対応 + 菌糸 SKILL Mode 5 新設 + 種 consolidate-wiki 03:50 active、wiki index 再生成 + 重複/矛盾検出 + 14 日 RAW archive、本文 append-only 厳格)/ **2 = Stage C 永続記憶**(bot.py に memory wiki + 過去3日 RAW + 当日 RAW を context ロード、ガクコが「昨日 X と話したよね」を覚える状態)/ **3 = SKILL 動的再読み込み**(mtime ベースの `_FileCache` / `_DirCache` / `_MemoryPastRawCache` で CHARTER/SKILL/PERSONA/memory 全部を bot 再起動なしで再ロード、S27 持ち越し消化)。永続記憶の構造が「対」(soil 正本ルール + memory 正本ルール)+「縦」(Stage A → A.5 → B → C)で揃った
- **直近セッション**: [2026-06-03 セッション30](../docs/sessions/2026-06-03-session30.md) — **永続記憶 3 ステージ active + SKILL 動的再読み込み**:S29 終了直後コールドスタートから「1→2→3 で行こう」指示で 3 案件完遂。**1**: launcher.mjs に `execute.model` → `--model` 渡し最小拡張(1.0.0 → 1.0.1)+ 菌糸 SKILL.md に Mode 5 = Consolidate 追記(index 再生成 + 重複/矛盾検出 + 14 日 RAW archive)+ 種 `consolidate-wiki.md` 起草(cron 03:50、engine claude-code、model claude-haiku-4-5)+ 初回実走 OK(wiki 2 ページ + index 再生成、重複/矛盾 0、archive 0)+ crontab 仕込み。**2**: bot.py に `MEMORY_BASE` 系定数 + 起動時 memory wiki + 過去 3 日 RAW + 当日 RAW を context ロード(30000 chars cap)+ `build_dialogue_prompt` に `[直近の記憶]` セクション挿入 + bot 再起動 → 初回起動成功 + サイズ確認 OK(計 ~22KB)。**3**: `_FileCache` / `_DirCache` / `_MemoryPastRawCache` の 3 つの mtime ベースキャッシュクラスを bot.py 内に実装 + persona/charter/skill/wiki/past_raw 全部をキャッシュ経由に書き換え + 起動時ウォームアップ + bot 再起動 → 全キャッシュ正常ロード(persona 826/charter 3839/skill 9817/wiki 3826/past_raw 4039)。Stage B/C の結果が bot 再起動を待たず次 turn で反映(対称性確保)
...
- **直近の重要決定**: S30 は新規 ADR なし(運用範囲、菌糸 SKILL Mode 5 と launcher 1.0.1 はそれぞれの README/SKILL 内で文書化)。前回 S29 = ADR 1 本 [2026-06-03 memory-source-of-truth](../docs/decisions/2026-06-03-memory-source-of-truth.md)
- **直近の宿題(最優先)**: **(今晩 22:40 / 明朝確認)** night-cheer が完了レポートを Discord に投げるか + log に NOTIFY ブロック残るか(S28 修正後の初検証)/ **(明朝 03:30 / 明朝確認)** ingest-raw が log に NOTIFY ブロックを残せるか / **(明朝 03:50 / 明朝確認)** **consolidate-wiki が cron 経由で初稼働**(S30 新設、index 再生成 + archive 0 件想定 + log NOTIFY)/ **(継続)** 測量士の次回手紙対応窓 / **(継続)** calendar token 寿命確認(約 6/5)/ **(継続)** kura 区画整備(S29 で failed の resolution 済 board 1 件目が退避候補確定)
