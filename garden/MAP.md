...
| 土壌-clients | [soil/clients/](soil/clients/) | 🌱 | **S48 新設・S49 横展開**:クライアント企業正本(2層 = 企業→案件、a〜f+メール+finance を一望)。[README](soil/clients/README.md) が型。**MTI 第1号縦通し**(Plaud+Gmail+見積/請求実物)+ **パナHM 横展開2号**(継続運営・Plaud 無し=メールのみで骨格)。世話役 = [client_steward](plots/client_steward/SKILL.md) |
...
| 区画 (plots) | [garden/plots/](plots/) | 🌱 | **第1号 daily-pilot(S19)+ 第2号 shift_manager(S21)+ メタ区画 plot_gardener(S33)+ 第3号 expense_processor(S35)+ 第4号 invoice_processor(S41)+ 第5号 field_assistant(S42)+ 第6号 sns_manager(S45)+ 第7号 finance(S47、3スキル統合)+ 第8号 client_steward(S48、クライアント世話役)**:いずれも CHARTER 継承型 |
...
| 区画-client_steward | [garden/plots/client_steward/](plots/client_steward/) | 🌱 | **S48 新設(seedling)→ S49 VPS デプロイ + test 昇格**:クライアント(toB)台帳の世話役。Mode B Bootstrap(横展開・`--domain` でドメイン直引き)/ Mode S Sweep(週次差分 digest)/ Mode R Brief(次段)。read-only digest(soil 書込は board → 剪定)。scope=master 一本(クライアント機密=構造遮断)。**MTI(第1号)+ パナHM(横展開2号)で型実証**。状態 **test**(初回発火 6/22 月08:20 見届けで active) |
