...
- **直近セッション**: [2026-06-17 セッション47](../docs/sessions/2026-06-17-session47.md) — **新区画 finance を transplant 移植(3スキル → 1区画)**: HMC finance_importer/analyzer/auditor を 1 区画に統合。重複回避 = freee_client 正本に読み取りメソッド追記のみ(Freee 連携・secret 新設なし)。3モード(I 記帳[部門ルール推定・Gemini 外す]/ D 監査[部門漏れ + 未登録明細検出、analyzer 前の地ならし役]/ A 分析[read-only 対話投げかけ])。月次サイクル 5日アップ→6日記帳→9日監査→10日分析。service + SKILL + 種3本 + bot 配線 + OPERATIONS Card 9。importer オフライン GREEN。状態 draft(VPS デプロイ待ち)。**+ セッション頭で sns_manager を active 昇格**(火6/16 IG+FB 実投稿着地)+ calendar token クリア(Production = 無期限)
