...
- **直近セッション**: [2026-06-14 セッション44](../docs/sessions/2026-06-14-session44.md) — **invoice_processor active 昇格**: 請求処理向け2名(河村・熊澤)の Freee 取引先登録を soil 反映(熊澤ページ新設 + 河村 freee_id 確定、VPS 照合検証 GREEN)+ Z番号がガクチョメモと逆と判明 → Freee 正本で記録 + README のスタッフ数過少カウント是正(安藤漏れ → active 31→33)+ ガクチョが5月分の承認一周完走 → **test→active**
...
| 区画 (plots) | [garden/plots/](plots/) | 🌱 | **第1号 daily-pilot(S19)+ 第2号 shift_manager(S21)+ メタ区画 plot_gardener(S33)+ 第3号 expense_processor(S35)+ 第4号 invoice_processor(S41)**:いずれも CHARTER 継承型 |
