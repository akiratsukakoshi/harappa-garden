...
- **実装フェーズ**: S25 で **連続失敗ガード(send_pending.py)+ 朝ブリーフィング今日フィルタ仕様明示化 + 測量士(外部視点 codex)導入 + `garden/OPERATIONS.md`(660 行)新設** を 1 日で完走。S24 で完成した board ライフサイクルに対して **救護網(連続失敗で auto-quarantine)+ 庭師用案内板(運用早見表)** を加えた構造。Garden が育つにつれ見通しが悪化していた問題に対して、外部視点 AI(測量士)を Garden 体制に組み込む新フェーズへ
- **直近セッション**: [2026-06-02 セッション25](../docs/sessions/2026-06-02-session25.md) — **連続失敗ガード + 仕様明示化 + 測量士導入 + OPERATIONS.md 新設**:Discord 通知 spam 即時止血(2 board を quarantine/cancelled) → `send_pending.py` に S25 連続失敗ガード実装(fail_count frontmatter + auto-quarantine to failed/ + 通知抑制、最大 2 通で打ち止め) / morning-briefing 今日フィルタ仕様の明示化(SKILL Step 3 + seed prompt 🔴 重要ブロック) / **測量士 = codex を外部視点として導入**(docs/surveyor/ + README + TEMPLATE + ADR + 語彙・CLAUDE.md/MAP/CHARTER 周知) / 測量士初回手紙到着 → 5 提案中 3 採用 1 保留(MAP 整理) → **OPERATIONS.md 新設(役割分担表 + 業務カード 4 枚 + HMC→HMG 移行マトリクス + 失敗時リファレンス)** で提案 1+2+4 を統合
- **直近の重要決定**: **連続失敗ガード設計**(board frontmatter に fail_count 記録、SEND_PENDING_FAIL_THRESHOLD default 3、status:test は隔離せず通知抑制のみ) / **測量士の位置づけ**(庭の中の役者ではない外部視点、docs/surveyor/ 配置、手紙は任意発火・1 ファイル統合方式・Claude 応答必須短文 OK) / **OPERATIONS と MAP の役割分担**(MAP は戦略地図、OPERATIONS は日々の運用早見表、CHARTER は plot 内規範、ADR は決定、session は履歴) / **大型実装より見通し整備を優先**(Stage A.5 を S26 以降に後倒し、S25 を見通しセッションに位置づけ)
- **直近の宿題(最優先)**: **(明朝 06:30 確認)** morning-briefing の今日フィルタ修正が active から `6/4 塚越北報告書`・`6/5 経費精算`・`6/8 ドラム缶かえす` を消すか / **(明日以降 1〜2 週)** OPERATIONS.md の使用感観察(機能 / 形骸化 / 手薄カード) / **(次セッション本命)** 永続記憶 Stage A.5 実装(S23 で 4 論点合意、S25 で見通し優先のため後倒し) / **(継続)** MAP 整理は OPERATIONS 観察後に段階移行(測量士提案 3 保留分) / **(継続)** 測量士の次回手紙対応窓(任意発火) / **(継続)** calendar token の 7 日後寿命確認(約 6/5)+ soil 正本関係の明文化(S23 持ち越し)
...
| 土壌-soil 全体配置 ← **NEW** | vault + garden-mirror + repo | 🌳 | **S23 vault 化**:S13 vault layout 設計穴を発見(soil 未配置)→ 案 1 採用、cp で vault に 65 ファイル + scp で garden-mirror に 65 ファイル配置 = memory と同パターン。正本関係明文化は次セッション宿題 |
| 記憶 (memory) | [garden/memory/](memory/) | 🌱 | **S22 Stage A 着手 + S23 Stage A.5 設計合意**:三層分離 ADR + master/raw/ RAW logging 稼働 + .gitignore で raw 除外 + **S23 で 4 論点合意(抽出粒度・主題判定・冪等・失敗時)+ ingest-raw skeleton 起草**。次=Stage A.5 実装(memory_logger.py 拡張 + wiki ディレクトリ作成 + dry-run + crontab 03:30)|
...
- [ ] **永続記憶 Stage A.5 実装**(S23 で 4 論点合意 + SKILL 詳細化 + skeleton 起草済、次回 memory_logger.py 拡張 + wiki ディレクトリ + dry-run + crontab 仕込み)
...
- [ ] **次回本命候補(1)**: **Stage A.5 実装**(memory_logger.py を `last_ingested_at` 対応 + memory/master/wiki/ ディレクトリ + index.md 雛形 + ingest-raw dry-run + crontab 03:30 仕込み + 初週運用観察)
- [ ] **次回本命候補(2)**: **repo / vault / garden-mirror の soil 正本関係を明文化**(S23 で 3 箇所配置になったが運用ルール未整理、必要なら ADR)
- [ ] **次回本命候補(3)**: 統合 Stage 1〜3(memory 移植 → 夜間バッチ → bot context ロード、Stage A.5 と並行 / 連動)
- [ ] **次回本命候補(4)**: SKILL+CHARTER 二段ロード稼働の継続観察(S20 〜 S23 で複数回稼働、トーン磨き込み判断)
...
- [ ] **次回本命候補(8)**: 菌糸 Mode 2 Lint(Stage 2、shift_manager 安定後)
