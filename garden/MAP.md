...
- **直近の宿題(最優先)**: **(sns)** S46 で初回フローを月曜前倒しで全段通し済(セレクト→文案→**本予約 IG+FB**)。**残 = 実投稿の着地確認 = 火 6/16 20:00 / 土 6/20 08:00 に IG+FB に実際に出たか**(失敗時 ig_scheduler が `error_msg`+メール通知。特に土は FB CDN URL 5日先 = 失効リスク要観察)→ test→active 昇格。週次フローの自然発火(土09:00 セレクト / 月07:30 文案)の初回も別途見届け / **(field)** 種 3 本の初回発火見届け = **weekly 6/15(月)08:10 / brief 次イベント D-2 朝 07:30 / furikae 6/30 19:30** → field_assistant test→active 昇格 / **(field メンション)** S43 で 4 名分有効化済(soil 記入 + sync 済)。**残 = 慶ちゃんがグループで一度発話 → `sync-line-users` 実行**(表示名は記入済みで自動マッチ) / **(invoice)** **S44 で active 昇格済**(5月分の承認一周をガクチョが完走。河村・熊澤の Freee 取引先登録 → soil 反映 → 承認一周)。**残 = 次回自動発火 7/12 08:00(6月分、external 込みの新フロー初通し)の見届け** / **(expense)** 本番1周(件数多めの月)をガクチョが見届け(extract → Sheet 編集 → 承認 → Freee 登録)/ **(shift)** finalize 種の初回発火見届け(**7/8 08:00**)→ active 昇格 / **(invoice 改善案・S43)** 給与メンバーの自動登録 = HMC `register_payroll.py`(稼働シート区分=給与 → 人事労務 freee 勤怠 API へ PUT、給与計算は人事労務側が自動)の Garden 移植を検討。会計 freee 経由は二重計上になるため不可、別ルートとして。HMC で現役稼働していたかはガクチョに要確認 / **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備 / 番人と launcher on_failure 連携(board/failed/ 自動通知)
...
| 区画 (plots) | [garden/plots/](plots/) | 🌱 | **第1号 daily-pilot(S19)+ 第2号 shift_manager(S21)+ メタ区画 plot_gardener(S33)+ 第3号 expense_processor(S35)+ 第4号 invoice_processor(S41)+ 第5号 field_assistant(S42)+ 第6号 sns_manager(S45)**:いずれも CHARTER 継承型 |
