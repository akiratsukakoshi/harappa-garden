...
- **直近の宿題(最優先)**: **(次フェーズ・本命)** **expense_processor Phase 2 → active**:`garden/services/expense-processor/` に processor.py 移植 + lib/freee_client(HMC `modules/freee_client/client.py` フル = post_deal/get_account_items/get_sections)+ lib/drive_client + Gemini 依存 → ⭐**secret 配置(ガクチョ console)**(Freee OAuth / `GEMINI_API_KEY` / Drive OAuth / `EXPENSE_DRIVE_FOLDER_ID`、600 perm)+ venv → dry-run 検証(**tax_code 経費=課税仕入 か要確認**)→ cron 登録(月末リマインド 28-31日19:00 / 抽出 2日08:00)→ 手動「経費まわして」確認 → 初回実走で draft→active / **(plot_gardener)** expense フル一周で test→active 昇格判断(Phase 2 初回実走を見届けてから上げるか) / **(LINE 本番昇格)** core_team 本来の read tool を選定してから ⭐本番グループ投入 → groupId 確定 → `.env`(`LINE_CORE_TEAM_GROUP_ID` + `LINE_TEST_USER_IDS` 空)→ `run-line-container.sh`(手順 = [DEPLOY.md](services/garden-gaku-co/DEPLOY.md) §8) / **(掃除)** 廃止 `run-line-server.sh` / `venv-line` 物理削除判断 / **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備
...
| 区画-expense_processor ← **NEW** | [garden/plots/expense_processor/](plots/expense_processor/) | 🌱 | **S35 新設 plots 第3号(plot_gardener 初 dogfood / transplant)**:CHARTER 継承 + Mode 1 月末リマインド(Drive URL つき)/ Mode 2 翌月2日抽出→board(cron+手動「経費まわして」・空ならスキップ)/ Mode 3 承認→dry-run→Freee 登録(Discord 直、send_pending 非経由)。HMC `apps/expense_processor/processor.py`(CSVParser/ImageParser/Gemini分類/post_deal)を継承。**scope=master 構造遮断**(core_team/staff に財務 tool を入れない)。状態 **draft**(Phase 2 = service 移植 + secret 待ち) |
| 種-expense_processor ← **NEW** | [garden/seeds/expense_processor/](seeds/expense_processor/) | 🌱 | **S35 種2本(draft)**:month-end-reminder(28-31日19:00 + is_last_day チェック、Drive URL runtime 構成)/ monthly-expense-draft(2日08:00 抽出 + 空判定 + board 起草、承認は discord_direct)。Phase 2 で service 移植 + secret + cron 登録後に active |
