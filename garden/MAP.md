...
- **直近セッション**: [2026-06-10 セッション41](../docs/sessions/2026-06-10-session41.md) — **新区画 invoice_processor を起草から test まで一気通貫(hybrid)**:HMC 移植(Gmail API 化 + genai 新 SDK)+ ★スタッフ照合 + ★稼働突合(請求漏れ検出)+ 12日種 cron 登録 + bot「請求書まわして」配線 + ⭐token/レビューWB + スモーク全 GREEN + **稼働突合の初成果 = スタッフマスターのずれ 4 件修正**(soil 31名化)。残 = 初回発火 7/12 見届け → active
...
- **直近の宿題(最優先)**: **(invoice)** 種 monthly-invoice-draft の**初回発火見届け(7/12 08:00、6月分)**→ active 昇格。Discord「請求書まわして」の実発話確認も初回時に / **(expense)** 本番1周(件数多めの月)をガクチョが見届け(extract → Sheet 編集 → 承認 → Freee 登録)/ **(plot_gardener)** **次セッションで新植型(seedling)を 1 本通す → OK なら test→active 昇格**(ガクチョ決定 S41。transplant/hybrid は expense・invoice で 2 回完走済、未検証は seedling モードのみ)/ **(shift)** finalize 種の初回発火見届け(**7/8 08:00**)→ active 昇格。明朝 06:30 = 番人サマリ入り morning-briefing の初ライブ確認 / **(LINE 本番昇格)** core_team 本来の read tool を選定してから ⭐本番グループ投入 → groupId 確定 → `.env`(`LINE_CORE_TEAM_GROUP_ID` + `LINE_TEST_USER_IDS` 空)→ `run-line-container.sh`(手順 = [DEPLOY.md](services/garden-gaku-co/DEPLOY.md) §8) / **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備 / 番人と launcher on_failure 連携(board/failed/ 自動通知)
