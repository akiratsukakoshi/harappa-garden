...
- **実装フェーズ**: S26 で **soil 正本ルール明文化(ADR + soil-sync スクリプト + セッションプロトコル)+ 永続記憶 Stage A.5 active 化(ingest-raw 種 + memory wiki + 冪等性修正 + crontab 03:30 仕込み)** を 1 セッションで完走。S25 で見通しを整備した翌セッション、本来 1 セッション 1 大型実装の予定が、地形整理(soil)と新基盤(菌糸 Mode 1 Ingest)を両方完走。Garden の記憶層が 03:00 + 03:30 の二段 cron(index-refresh → ingest-raw)で自律維持される構造に到達
- **直近セッション**: [2026-06-02 セッション26](../docs/sessions/2026-06-02-session26.md) — **soil 正本ルール + Stage A.5 active 化**:S23 持ち越し宿題「soil 3 箇所配置(repo/vault/VPS)の正本関係明文化」を **層別正本モデル**(repo = 構造ファイル / VPS = 菌糸ログ / vault = 読み取り専用ビュー)で確定 + `garden/services/soil-sync/{pull/push}` 2 本実装 + セッション開始時 pull / 終了時 push をプロトコル化 → **Stage A.5 (菌糸 Mode 1 Ingest)** に着手:wiki ディレクトリ + index 雛形 + ingest-raw seed VPS 配置 + dry-run 4 回(1 回目で実走成功、2-3 回目で turn 時刻比較の冪等性脆弱を発見、**RAW 単位の処理完了マーカーモデル**に prompt 修正、4 回目で完全 skip 確認)+ crontab 03:30 仕込み + 副次発見で memory も 3 箇所配置問題(VPS に README.md なし → S27 宿題)
- **直近の重要決定**: **soil の層別正本**(repo = staff/business/workflows/clients/events/projects/meetings/concepts の正本 / VPS = log.md + index.md の菌糸産正本 / vault = Obsidian 読み取り専用ビュー、ガクチョ自身が「直接触らない運用ですっきりする」と表明 → 案 A 棄却、層別案採用) / **soil-sync 同期スクリプト 2 本**(pull-from-vps セッション開始時 / push-to-vps 編集後、`--delete` なしで初版開始、1〜2 週運用観察) / **Stage A.5 冪等性は RAW 単位**(ADR §論点 3「turn 単位」→「RAW 単位の処理完了マーカー」に変更、session23 サマリに補注で記録、turn 時刻比較の脆弱性を dry-run で発見) / **今後 memory も同期 ADR 必要**(soil とは違う構造 = raw は .gitignore 除外、構造ファイルは 3 箇所、要別設計)
- **直近の宿題(最優先)**: **(明朝 03:30 確認)** ingest-raw cron 初稼働で 6/2 RAW(本セッション分含む)が処理されて wiki に反映されるか / **(明朝 06:30 確認)** morning-briefing の今日フィルタ修正(S25 仕込み)が active から `6/4 塚越北報告書`・`6/5 経費精算`・`6/8 ドラム缶かえす` を消すか / **(明日以降 1〜2 週)** OPERATIONS.md 使用感観察 + ingest-raw 初週運用観察(捨て率・主題揺れ・wiki 重複の傾向) / **(継続)** memory 正本ルール ADR + memory-sync スクリプト(S26 内発見、S27 宿題) / **(継続)** MAP 整理は OPERATIONS 観察後に段階移行 / **(継続)** 測量士の次回手紙対応窓(任意発火) / **(継続)** calendar token の 7 日後寿命確認(約 6/5)
...
| 区画-daily-pilot | [garden/plots/daily-pilot/](plots/daily-pilot/) | 🌳 | **S19 立ち上げ / S20 CHARTER 継承化 / S25 今日フィルタ明示化**:HMC hmc_pilot 起源 + Triage 3軸再設計 + active→backlog 反映 Mode 3 明示 + frontmatter `topics:` declare(越境 picker 用)+ **S25: Step 3 に「Step 1 で抽出した `deadline ≦ today` のみを active に書く」を明示**(seed prompt にも 🔴 重要ブロック追加) |
...
| 配信ディスパッチャ | [services/garden-gaku-co/send_pending.py](services/garden-gaku-co/send_pending.py) | 🌳 | **S21 新設 + S24 大幅拡張 + S25 連続失敗ガード**:cron 1分毎、status: approved 検知 → dispatch、 **status: pending → 承認依頼通知(notified_at 冪等化)、shell 種成功時 → 連鎖 confirmation の unblock**、scheduled_send 待機、status:test テスト配信。**S25: fail_count frontmatter + auto-quarantine(N 回失敗で failed/ へ自動退避)+ 通知抑制(spam 構造の根絶)** |
