...
- **実装フェーズ**: S21 で **shift_manager の Garden 完結化** を一気通貫(plots 第2号 SKILL + 種3本 + Python service 移植 + post_approval 経路 + コドモン CSV 取込 + cron 4本仕込み)。月末月初の本番初発火を **5/31 22:00** に控える(2026-06 月初運用が初の Garden 上での月次サイクル)
- **直近セッション**: [2026-05-30 セッション21](../docs/sessions/2026-05-30-session21.md) — **shift_manager Garden 化 + 月末月初 active 化**:HMC `apps/shift_manager/logic/` の2本(generate_shift_form / generate_working_hours)+ freee_client(get_partners 最小版)+ utils を Garden に移植 / plots 第2号 SKILL.md 起草(CHARTER 継承)/ 種3本(monthly-shift-survey v2 + month-end-working-hours-prep + monthly-working-hours-confirmation) / send_pending.py 新規実装((4) C 案、cron 1分毎・scheduled_send/status:test 対応) / コドモン CSV 自動取込(import_kodomon.py + run_month_end_collect.sh) / VPS credentials 配置 + API 疎通検証 + cron 4本仕込み
- **直近の重要決定**: **(4) post_approval 検知 = C 案**(send_pending.py の cron 1分毎、scheduled_send で時刻待機)/ **(3) Mode 3 = URL 投稿方式**(LINE Bot API は PDF 直送不可、スクショ自動化はフォント崩れリスク、URL 方式が現実解)/ **配信タイミング = 19:00 統一**(シフト + 稼働 同時、ガクチョの日中作業時間確保)/ **テスト配信 = status: test → personal LINE**(本配信前の文面検証、何度でも繰り返し可)/ **HMC との並行運用回避**(OAuth race 防止、Garden に統一)
- **直近の宿題(最優先)**: **(5/31 22:00 本番初発火)** month-end-working-hours-prep が board 起草 → ガクチョが夜〜翌朝に集計実行 → 6/1 08:00 cron 2本起草 → 19:00 自動配信 / **(継続宿題)** NPM 経由 bot.harappa.monster の認証(現状ノー認証、staff LINE 投稿経路) / **(次セッション)** テスト配信実機検証 + コドモン CSV 取込検証 + SKILL+CHARTER 二段ロード初稼働の体感観察(まだ S20 から積み残し)+ 菌糸 Stage 1 + 永続記憶 Stage A
