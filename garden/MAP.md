...
- **実装フェーズ**: S23 で **菌糸 Stage 1 active(Mode 3 Index 更新)+ Stage A.5 設計合意(4 論点 D/C/C/A)+ soil の vault 化(LiveSync 同期エリアに昇格)+ 6/1 月初配信の 1 ヶ月ずれ修正** を一気通貫。Mycelium SKILL + 種 index-refresh + crontab 03:00 が仕込み済、**明朝 6/1 03:00 から本稼働開始**。種 ingest-raw skeleton 起草で次セッション実装ベース固まる
- **直近セッション**: [2026-05-31 セッション23](../docs/sessions/2026-05-31-session23.md) — **菌糸 Stage 1 active + Stage A.5 設計合意 + soil vault 化 + monthly-shift-survey 月ずれ修正**:事前確認(アンケート対象月 +2→+1 月修正 + 関連ファイルの「翌々月」呼称統一)+ recurring_master 月末項目分割(r010 リネーム + r016 新規=シフトアンケート回答チェック + コドモンCSVを設置)/ Mycelium SKILL.md 起草(Mode 1〜4 集約)/ soil/index.md 初回 full scan(staff 29 / business 18 / workflows 4 / concepts 1 の意味的サマリ)/ vault layout 設計穴の発見(soil が vault に未配置)→ 案 1(soil を vault に追加 = LiveSync 同期)採用 + 65 ファイル配置 / 種 index-refresh.md 起草 + dry-run フェーズ 1 完走(launcher exit 0、62 件検知、index-bootstrap で全件カバー済を LLM が柔軟判断)+ crontab 03:00 仕込み / Stage A.5 4 論点合意(D/C/C/A)→ SKILL Mode 1 詳細化 + ingest-raw skeleton 起草
- **直近の重要決定**: **菌糸 Stage 1 active**(Mode 3 Index 更新、毎日 03:00 cron、自律実行) / **soil の置き場 = vault + garden-mirror + repo の 3 箇所同期エリア**(memory と同パターンに統一、正本関係の明文化は次セッション宿題) / **Stage A.5 4 論点**(D: 抽出ハイブリッド=短文事実→staff ページ・長文→log.md、C: 主題判定ハイブリッド=事前定義7+LLM 新規命名、C: 冪等=RAW 単位 last_ingested_at + wiki append-only、A: 失敗時=ADR §2「グレー捨てる」厳格適用) / **monthly-shift-survey 対象月 = +1 月**(6/1 発火 → 7 月分。「翌々月」呼称はすべて「翌月」に統一、Mode 1 月末は「翌日 Mode 2 の対象月」と相対参照)
- **直近の宿題(最優先)**: **(5/31 22:00 本番初発火)** month-end-working-hours-prep が board 起草 → 集計実行 / **(6/1 03:00 菌糸初稼働)** index-refresh の自動実行結果確認(soil/log.md エントリ追加) / **(6/1 19:00 dummy 配信)** 種2本 → Discord master プレビュー流入 → ガクチョ手動で LINE staff グループへコピー / **(次セッション)** Stage A.5 実装(memory_logger.py 拡張 + wiki ディレクトリ + ingest-raw dry-run + crontab 03:30 仕込み)+ soil 正本関係の明文化 / **(継続宿題)** NPM 認証懸念(統合 Stage 6 で同時解消予定)+ calendar token の 7 日後寿命確認(約 6/5)
...
- [x] **菌糸 Stage 1(Mode 3 Index 更新)実装**(S23 active、明朝 6/1 03:00 初稼働)
