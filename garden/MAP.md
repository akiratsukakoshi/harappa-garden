...
| 区画-expense_processor | [garden/plots/expense_processor/](plots/expense_processor/) | 🌳 | **S35 新設 → S37 Phase 2 + 初回実走で active**:Mode 1 月末リマインド / Mode 2 抽出→board / Mode 3 承認→dry-run→Freee 登録。**scope=master 構造遮断**。状態 **active**(S37 初回実走 = 実明細 extract → ガクチョが CSV 直接レビュー → Freee 59件 ¥397,373 登録。partial 失敗ハンドリング[FAILED 退避+exit1+==NOTIFY==]実装)。残=Discord「経費まわして」配線 / 分類バッチ化 |
