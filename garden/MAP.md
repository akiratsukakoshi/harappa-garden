...
- **直近の宿題(最優先)**: **(次フェーズ・本命)** テスト環境(ガクチョ 1:1)で **区画(plot)の read tool 群を実装 → 確認 → 本番昇格**。plot_gardener(S33)を通して dogfood 第 1 号を選定(候補: core_team read tool / shift_manager / expense_processor)。実ツール実装後、1:1 でも tool-use 再開 / **(本番昇格)** 区画確認後 ⭐本番グループに bot 投入 → groupId 確定 → `.env`(`LINE_CORE_TEAM_GROUP_ID` 記入 + `LINE_TEST_USER_IDS` 空に)→ `run-line-container.sh`。手順 = [DEPLOY.md](services/garden-gaku-co/DEPLOY.md) §8 / **(掃除)** 廃止した `run-line-server.sh` / `venv-line` の物理削除判断(無害なので保留可)/ **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備
