...
- **直近セッション**: [2026-06-11 セッション42](../docs/sessions/2026-06-11-session42.md) — **新区画 field_assistant(seedling 初適用)を起草から本番稼働まで一気通貫**:要件整理(未整理依頼 → Workflow Spec)+ 種 3 本 cron + core_team 初 tool 2 本(名簿/天気[風向・地理院ジオコーディング])+ ⭐LINE 本番グループ投入・稼働開始 + メンション基盤(webhook userId 収集 + soil line_display_name)+ 1:1 恒久テスト化 + morning_greet Triage 欠落バグ修正 + 番人朝統合初ライブ GREEN + **plot_gardener active 昇格**
...
- **直近の宿題(最優先)**: **(field)** 種 3 本の初回発火見届け = **weekly 6/15(月)08:10 / brief 次イベント D-2 朝 07:30 / furikae 6/30 19:30** → field_assistant test→active 昇格 / **(field メンション)** 運営メンバーがグループで一度ずつ発話 → `sync-line-users` 実行 + soil `line_display_name` 埋め → メンション有効化 / **(invoice)** 種 monthly-invoice-draft の**初回発火見届け(7/12 08:00、6月分)**→ active 昇格。Discord「請求書まわして」の実発話確認も初回時に / **(expense)** 本番1周(件数多めの月)をガクチョが見届け(extract → Sheet 編集 → 承認 → Freee 登録)/ **(shift)** finalize 種の初回発火見届け(**7/8 08:00**)→ active 昇格 / **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備 / 番人と launcher on_failure 連携(board/failed/ 自動通知)/ 佐倉フィールドの正確な座標(現在は佐倉市中心の概算)
...
| 土壌-people/staff | [soil/people/staff/](soil/people/staff/) | 🌳 | 31名 active(S41 で 2 名追加 + 表記正字化)+ **S42: 運営 5 名に `line_display_name` 欄新設**(メンション紐づけ用、値はグループ発話収集後に確定)。alumni候補48は保留 |
...
| 本番ランチャー | [garden/services/launcher/](services/launcher/) | 🌳 | **S13 完動 + S14 night-review 実走成功 + S15 で cron 無人実走を実証 + S30 で 1.0.1 拡張**(`execute.model` frontmatter → `claude -p --model {model}` で渡す、claude-haiku-4-5 等を種別に指定可能)。cron 化済(06:25/06:30/22:30/03:00/03:30/03:50)。**S40: unit test 12 ケース(`npm test`、本体無改修・dry-run 方式、実運用種 3 本の退行検査込み)** |
