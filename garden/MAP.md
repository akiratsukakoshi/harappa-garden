...
- **実装フェーズ**: S27 で **放サボ上書き事故対応 → 構造改革(board/log を vault 外へ移行 + Discord ガクコ承認運用 + daemon EXCLUDE_PREFIXES)** を 1 セッションで完走。事故発端(LiveSync 起因の board 巻き戻しによる稼働サマリ放サボ列消失)から、Obsidian vault は知識ベース(soil/inbox/memory)のみ、board/log は VPS 単独管理という抜本的な配置整理に到達。承認運用は Obsidian 編集 → Discord 自然言語(ガクコ → board frontmatter Edit)に切り替わり、ダミーテストで動作確認済
- **直近セッション**: [2026-06-02 セッション27](../docs/sessions/2026-06-02-session27.md) — **放サボ上書き事故 + 構造改革**:5 月稼働サマリの放サボ列消失(LiveSync 経由 board 巻き戻し → send_pending 重複実行)を発見 → import_kodomon 手動実行で 12 セル復元 → 構造改革:**board/log を vault 外 `/home/vps-harappa/garden/{board,log}/`** + **mirror/writeback daemon に EXCLUDE_PREFIXES**(LiveSync ignore の代替、より頑健)+ **shift_manager SKILL Mode 5 新設**(Discord 承認応答ルール)+ **bot.py に list_pending_boards 注入**(Bash/Glob 禁止下の pending 認識)+ **send_pending.py notify_pending 強化**(配信本文プレビュー + 客観事実 + ガクコ操作ガイド)+ **board_facts.py 新規**(seed 別事実プロバイダ)+ **既存タブガード**(`generate_working_hours.py` の `--force-regenerate` + `_has_saboru_data`)+ **CSV パス検出を `import_kodomon.resolve_csv_path` に一本化** + ADR 新規 + CouchDB 27 件 bulk delete + ダミー board で総合テスト成功(`status: approved` 書き換え確認)
- **直近の重要決定**: **board/log は Obsidian vault の外で管理**(LiveSync 起因の構造的事故を構造的に封じ込め、`garden-mirror/` = 共有知識ベース / `garden/` = VPS スクリプト領域、関心分離)/ **承認運用 = Discord ガクコ経由**(Obsidian 編集 UI を経由しない、ガクコが SKILL Mode 5 通り自然言語を解釈して board を Edit)/ **daemon EXCLUDE_PREFIXES 機構**(VPS 側の単一真実、Obsidian UI 設定に依存しない頑健な隔離、別端末増加にも自動対応)/ **generate_working_hours.py 既存タブガード**(放サボ列にデータあり時は `--force-regenerate` 必須、同じ事故の構造再発ゼロ)
- **直近の宿題(最優先)**: **(明朝 03:30 確認)** ingest-raw cron が S26-S27 RAW を処理して wiki に反映するか(S26 持ち越し)/ **(明朝 06:30 確認)** morning-briefing が **新パス** `/home/vps-harappa/garden/board/pending/` を見にいくか + S25 仕込みの今日フィルタ動作 / **(明日以降 1〜2 週)** ingest-raw / OPERATIONS.md / Discord 承認運用の使用感観察 / **(次セッション本命)** memory 正本ルール ADR + memory-sync スクリプト(S26 内発見、S27 持ち越し)/ **(継続)** bot.py の SKILL 動的再読み込み機構(今回の bot 再起動忘れの再発防止)/ **(継続)** 測量士の次回手紙対応窓 / **(継続)** calendar token の 7 日後寿命確認(約 6/5)
