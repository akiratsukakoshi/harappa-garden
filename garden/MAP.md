...
- **直近セッション**: [2026-06-12 セッション43](../docs/sessions/2026-06-12-session43.md) — **実運用×修正の同時進行**: field_assistant 修正3点(メンション/ストアズなし/名前抽出+チケット名)+ invoice 初回発火 exit 143 → launcher timeout_minutes & stale lock 根治(テスト14/14)→ 再実行完走(稼働突合初成果=塚越)+ 5月分実運用ラリー(飯田過去分根治/経営除外/内山表記ゆれ/数値セル化/税区分プルダウン)+ 大阪2名 invoice_monthly + external 移植(外部スタッフ稼働分 6行 ¥76,583)
...
- **直近の宿題(最優先)**: **(field)** 種 3 本の初回発火見届け = **weekly 6/15(月)08:10 / brief 次イベント D-2 朝 07:30 / furikae 6/30 19:30** → field_assistant test→active 昇格 / **(field メンション)** S43 で 4 名分有効化済(soil 記入 + sync 済)。**残 = 慶ちゃんがグループで一度発話 → `sync-line-users` 実行**(表示名は記入済みで自動マッチ) / **(invoice)** 初回発火は S43 で完走済(5月分)。**残 = 5月分の承認一周見届け**(ガクチョ: MISMATCH 修正 + 河村思依蕗・熊澤満穂の Freee 取引先登録 → Discord「承認」→ Mode 2 dry-run → 本登録)→ active 昇格判断。次回自動発火 7/12 08:00(6月分、external 込みの新フロー初通し) / **(expense)** 本番1周(件数多めの月)をガクチョが見届け(extract → Sheet 編集 → 承認 → Freee 登録)/ **(shift)** finalize 種の初回発火見届け(**7/8 08:00**)→ active 昇格 / **(invoice 改善案・S43)** 給与メンバーの自動登録 = HMC `register_payroll.py`(稼働シート区分=給与 → 人事労務 freee 勤怠 API へ PUT、給与計算は人事労務側が自動)の Garden 移植を検討。会計 freee 経由は二重計上になるため不可、別ルートとして。HMC で現役稼働していたかはガクチョに要確認 / **(継続)** calendar token 寿命 / 測量士の次回手紙 / ingest-raw `==NOTIFY==` 初検証 / kura 整備 / 番人と launcher on_failure 連携(board/failed/ 自動通知)/ 佐倉フィールドの正確な座標(現在は佐倉市中心の概算)
...
| 土壌-people/staff | [soil/people/staff/](soil/people/staff/) | 🌳 | **32名 active**(S43: 安藤寛人新設[大阪])+ S43: line_display_name 4名記入(メンション有効化)+ **新フラグ `invoice_monthly`**(稼働シート外でも毎月請求 = check 突合対象、大阪2名)。alumni候補48は保留 |
...
| 区画-invoice_processor ← **NEW** | [garden/plots/invoice_processor/](plots/invoice_processor/) | 🌱 | **S41 新設 → 同セッションで test まで(plot_gardener 2nd dogfood、hybrid 改植)**:HMC `apps/invoice_processor/` 継承(fetch/extract/register + MISMATCH 補正)+ ★新機能 2 本 = **soil スタッフ照合**(請求元を staff/リスト外に判定)+ **稼働突合**(`{前月}_稼働時間` 区分=業務委託 と突合 → 請求漏れ検出)。毎月 12 日 cron + 手動「請求書まわして」(bot 配線済)。scope=master 構造遮断。状態 **test**(**S43: 初回発火完走**[exit 143 → timeout 30分化で再実行。23ファイル87行、稼働突合初成果=塚越検出]+ 実運用ラリーで SKILL に運用ノウハウ蓄積 + **税区分プルダウン** + **external 統合**。**残 = 5月分の承認一周 → active 判断**) |
