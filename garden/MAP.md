...
- **実装フェーズ**: S29 で持ち越し本命 + 学びの言語化 + 掃除を 3 連で完遂。**A = memory 正本ルール ADR + memory-sync スクリプト 2 本**(S26-S28 持ち越し本命を消化、soil-sync の対称設計、`--exclude='*/raw/*.md'` で raw は VPS 専属を構造保証)/ **B = OPERATIONS.md §5 path 大移動チェックリスト新章**(S28 学び由来、6 層構成 + 副作用ゼロ完了検証 + 24h 後検証ポイント宿題化の鉄則)/ **C = board 掃除**(failed 1 件は resolution note + 残置、quarantine 2 件はテスト残骸削除)。soil と memory の正本ルールが「対」として揃った
- **直近セッション**: [2026-06-03 セッション29](../docs/sessions/2026-06-03-session29.md) — **memory 正本ルール ADR + path 大移動チェックリスト + board 掃除**:コールドスタートから 3 案件を A→B→C で完遂。**A**: ADR 新規 + memory-sync 2 本(pull/push)+ README + memory README に正本ルール表追記 + CLAUDE.md セッションプロトコル更新 + dry-run / 実走検証 OK。**B**: OPERATIONS.md §5 新章(コード/設定/種/ドキュメント/daemon/完了検証 の 6 層 + 5 分後ふりかえり、既存 §5 は §6 へ繰り下げ)。**C**: failed の `2026-06-01-monthly-shift-survey.FAILED.md` に resolution フィールド追加(S24 で再キック本配信成功済を明示)+ quarantine 2 件削除(`_test-dummy.md` / `2026-06-02-dummy-test-s27.md`)
- [2026-06-03 セッション28](../docs/sessions/2026-06-03-session28.md) — **S27 path 移動取りこぼし 2 箇所修正**:昨夜 22:40 Discord 警告通知から原因 2 段階特定 →(1) `night_cheer.py:51` の `read_review_log` が旧パスのまま (2) settings.json に新 log path の Write/Edit 権限なし → 修正 + 副次発見として ingest-raw も同じ permission 問題で同時解決 + 03:30 ingest-raw のコア処理は成功していて wiki に 1 件追記実績(Stage A.5 初収穫)
- **直近の重要決定**: ADR 1 本 = [2026-06-03 memory-source-of-truth](../docs/decisions/2026-06-03-memory-source-of-truth.md)(memory は repo/vault/VPS 3 箇所配置、構造ファイル=repo / wiki=VPS 主・repo 従 / raw=VPS 専属、soil-sync 対称設計で別ディレクトリに責務分離、採用しなかった案 3 つも記録)。OPERATIONS.md §5 path 大移動チェックリストは ADR 起票せず、運用早見表の章として位置付け(作業手順は運用書の方が自然との判断)
- **直近の宿題(最優先)**: **(今晩 22:40 / 明朝確認)** night-cheer が完了レポートを Discord に投げるか + log に NOTIFY ブロック残るか / **(明朝 03:30 / 明朝確認)** ingest-raw が log に NOTIFY ブロックを残せるか(S28 修正後の初検証)/ **(継続)** bot.py の SKILL 動的再読み込み機構(S27 持ち越し)/ **(継続)** Stage B(夜間バッチ + 14 日経過 RAW 削除)/ Stage C(bot 起動時 context ロード)/ **(継続)** 測量士の次回手紙対応窓 / **(継続)** calendar token 寿命確認(約 6/5)/ **(継続)** kura 区画整備(S29 で failed の resolution 済 board 1 件目が退避候補確定)
...
| 記憶 (memory) | [garden/memory/](memory/) | 🌳 | **S22 Stage A 着手 → S23 設計合意 → S26 Stage A.5 active 化 → S29 正本ルール明文化**:三層分離 ADR + master/raw/ RAW logging 稼働 + .gitignore で raw 除外 + S23 4 論点合意 + S26 で wiki ディレクトリ + ingest-raw 種 active(VPS cron 03:30) + RAW 単位の冪等性 + **S29 正本ルール ADR**(構造ファイル=repo / wiki=VPS 主・repo 従 / raw=VPS 専属)+ memory-sync 2 本 + 03:34 ingest-raw 初収穫実績(`tech_infra.md` に S27 ダミーテスト承認 1 件追記)。次=Stage B(夜間バッチ + 14 日 RAW 削除)+ Stage C(bot 起動時 context ロード) |
...
- [ ] **次回本命候補(1)**: 統合 Stage B(夜間バッチ EXTRACT + CONSOLIDATE、03:00 JST、claude-haiku-4-5)+ 14 日経過 RAW 削除
- [ ] **次回本命候補(2)**: 統合 Stage C(bot 起動時に master index + 最近 RAW を context にロード = 真の永続記憶)
- [ ] **次回本命候補(3)**: bot.py の SKILL 動的再読み込み機構(S27 持ち越し)
...
- [ ] **次回本命候補(11)**: kura 区画整備(S29 で failed の resolution 済 board 1 件目が退避候補確定、月次移行ポリシー設計)
