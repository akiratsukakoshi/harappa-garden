...
- **このファイル(`CLAUDE.md`)はエージェントの入口ファイル名として残すが、内容は中立な役割名で書く**(「Claude が」→「実装エージェントが」等)。Garden の起動層は launcher の `engine` / **Garden Runtime(seed runner)** 抽象で切替可能(測量士 2026-06-24 提案5)。
...
| 蔵 (Kura) | 長期アーカイブ |
| 測量士 (Land Surveyor) | 外部視点 — codex(別系列LLM)が庭の縁から定期俯瞰、手紙で報告。測量士の役割中は実装しない(codex は将来 実装者にもなり得る。同一セッションでは役割を明示する)([docs/surveyor/](docs/surveyor/)) |
...
- `docs/surveyor/` — **測量士(codex)からの手紙 + 実装エージェント(現状 Claude Code)の応答**。外部視点のレビュー記録。運用ルールは [`docs/surveyor/README.md`](docs/surveyor/README.md)
...
**終了時**: 塚越さんから合図(「進捗書いて」「ここで止める」「セッション閉じる」「いったん区切る」等)があれば、実装エージェント(現状 Claude Code)が以下を実行:
...
3. `README.md` を確認し、公開向けの全体像(コンセプト・語彙・実装済み機能・ローカル/VPS 関係・チャンネル・soil・ディレクトリ構造)に影響する変更があれば更新する。公開 repo なので個別クライアント・個人・組織・実 URL・secret 値は載せない
4. `garden/MAP.md` を最新化(現在地・区画ステータス・宿題・直近セッション欄)
5. `docs/sessions/YYYY-MM-DD-N.md` を作成(セッションサマリ、N は当日通し番号)
6. 内容を塚越さんに提示
7. 塚越さん OK で完了(git commit は別途指示)

進捗管理の粒度(粗→細):
- `README.md` — 公開向けの全体像(外部に出せる抽象度)
